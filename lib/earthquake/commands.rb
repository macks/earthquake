...
Earthquake.init do
  ...
  command %r|^:recent\s+([^\s]+)\/([^\s]+)$|, :as => :recent do |m|
    puts_items twitter.send(:get, "/lists/statuses.json?slug=#{m[2]}&owner_screen_name=#{m[1]}&include_rts=1&include_entities=1")
  end
  ...
  command :user do |m|
    ...
    ap user.slice(*%w(id_str screen_name name profile_image_url description url location time_zone lang protected statuses_count followers_count friends_count listed_count created_at following))
  end
